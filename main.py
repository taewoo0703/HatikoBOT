...
async def hatiko(order_info: MarketOrder, background_tasks: BackgroundTasks):
    nMaxLong = 1
    nMaxShort = 1
    hatikoBase(order_info, background_tasks, nMaxLong, nMaxShort)
...
def hatikoBase(order_info: MarketOrder, background_tasks: BackgroundTasks, nMaxLong: int, nMaxShort: int):
    """
    /hatiko
    [기존 /hatiko 대비 변경내역]
    1. 매수종목을 리스트로 관리함. 리스트에 최대개수 이상 들어오는 시그널은 무시. 혹시 같은 종목에서 Long1 시그널 중복 발생시 후속타 무시
    2. bot.get_amount() 대신 bot.get_amount_hatiko()를 씀. 트뷰와 무관하게 그냥 전체 자금 기준 n분할하는 방식으로 매매 수량을 정함.(청산당할MDD보정 포함되어 있음)
    """
                ...
                if order_info.side.startswith("entry/"):
                    if order_info.stop_price and order_info.profit_price:
                        ...
                        if nTry == 0:   # 초기 세팅
                            ...
                            total_amount = bot.get_amount_hatiko(order_info.base, quote, nMaxLong, nMaxShort)
            ...
            if order_info.order_name in nextSignal_list:
                ...
                for orderID in orderID_list_old:
                    ...
                    if not isCancelSuccess:
                        resultCancel = bot.future.cancel_order(orderID, symbol)
